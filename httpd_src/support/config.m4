...
if test "$enableval" = "yes" ; then
  ...
  APR_ADDTO(httest_LTFLAGS, [-static])
...
AC_ARG_ENABLE(static-httest,APACHE_HELP_STRING(--enable-static-httest,Build a statically linked version of httest),[
if test "$enableval" = "yes" ; then
  APR_ADDTO(httest_LTFLAGS, [-static])
else
  APR_REMOVEFROM(httest_LTFLAGS, [-static])
fi
])
APACHE_SUBST(httest_LTFLAGS)
